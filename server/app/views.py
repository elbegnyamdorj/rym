...
class GroupMore(APIView):
    ...
    def get(self, request, format="json"):
        data = request.query_params["id"]
        try:
            groups = Group.objects.get(id=data)
        ...
        if serializer.is_valid():
            ...
            for i in stud_list:
                user = Users.objects.get_or_create(
                    ...
                    defaults={
                        "email": i,
                        "user_type_id": UserType.objects.get(user_type_name="Оюутан"),
                    },
                )
                if user:
    ...
    def get(self, request, format="json"):
        ...
        try:
            ...
            for i in team_members:
                student = GroupStudents.objects.filter(id=i["group_student_id"])
                student_serializer = GroupStudentSerializer(student, many=True)
                student_info = dict(student_serializer.data[0])
                more_info_student = Users.objects.filter(id=student_info["student_id"])
                s_more_info_serializer = UserSerializer(more_info_student, many=True)
            ...
            final_data = [
                {
                    ...
                    "email": i["student_info"]["email"],
                    "first_name": i["student_info"]["first_name"],
                    "last_name": i["student_info"]["last_name"],
                }
                for i in team_members
            ]
            d = {}
    ...
    def post(self, request, format="json"):
        ...
        for i in raw_list:
            for j in i["selected"]:
                updated_list.append(
                    {
                        "team_name": str(i["id"]) + "-р баг",
                        "subgroup_id": subgroup_id,
                        "group_student_id": GroupStudents.objects.filter(
                            student_id=j["value"], group_id=group_id
    ...
    def get(self, request, format="json"):
        ...
        try:
            ...
            team = TeamMember.objects.get(
                group_student_id=GroupStudents.objects.get(
            ...
            team_members = TeamMember.objects.filter(
                subgroup_id=student_info["subgroup_id"],
                team_name=student_info["team_name"],
            ).values()
            for i in team_members:
                std_id = GroupStudents.objects.filter(
                    id=i["group_student_id_id"]
                ).values()[0]["student_id_id"]
...
class GroupRatings(APIView):
    ...
    def get(self, request, format="json"):
        ...
        team_member = TeamMember.objects.filter(
            group_student_id__group_id=group_id, subgroup_id=subgroup_id
        ).values()
        for i in team_member:
            rating = Ratings.objects.filter(team_member_id=i["id"]).values(
                "rating_value"
            )
...
class LessonRatings(APIView):
    ...
    def get(self, request, format="json"):
        ...
        for i in group_id_list:
            ...
            group_students = GroupStudents.objects.get(
                group_id=i, student_id=student_id
            )
            ...
            for j in sg_serializer.data:
                teams = TeamMember.objects.filter(
                    subgroup_id=j["id"], group_student_id=group_student_id
                )
                t_serializer = TeamMemberSerializer(teams, many=True)
                for z in t_serializer.data:
                    rating = Ratings.objects.filter(team_member_id=z["id"]).values_list(
                        "rating_value"
                    )
                    rating_list = [item for t in rating for item in t]
            ...
            group_info.append(
                {
                    "lesson_name": g_data["lesson_name"],
                    "group_id": g_data["id"],
                    "rating_value": avg,
                    "criteria_rating": criteria_list,
                    "good_comm": good_comm,
                    "bad_comm": bad_comm,
                }
            )
...
class Rated(APIView):
    ...
    def post(self, request, format="json"):
        ...
        try:
            my_rated_mates = Ratings.objects.filter(
                team_member_id__in=team_member_id, rated_by_id=user_id
            )
...
class Rating(APIView):
    ...
    def post(self, request, format="json"):
        ...
        comment = Comments.objects.create(bad_comm=bad_comm, good_comm=good_comm)
        for i in data:
